...
async function getStore(): Promise<DbPuppetStore> {
	const store = new Store({
		filename: ":memory:",
	} as any);
	await store.init();
	return new DbPuppetStore(store.db);
}

describe("DbPuppetStore", () => {
	it("should handle mxid info", async () => {
	...
	it("should handle puppet info", async () => {
		const store = await getStore();
